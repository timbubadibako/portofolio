...
export function EducationSection({ mode = 'gui' }: { mode?: 'gui' | 'terminal' }) {
  ...
  useEffect(() => {
    async function fetchChronology() {
      try {
        ...
        setEducation(data.filter((item: any) => item.type === "EDU"))
      } catch (err) {
  ...
  useEffect(() => {
    if (!isLoading && visibleCount < education.length) {
      const timer = setTimeout(() => {
        setVisibleCount(prev => prev + 1)
      }, 300)
      return () => clearTimeout(timer)
    }
  }, [visibleCount, education.length, isLoading])
          ...
          >
            ...
            <div className="space-y-1">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-white font-bold text-sm tracking-tight">{edu.role}</span>
               </div>
