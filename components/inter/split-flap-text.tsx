...
function SplitFlapChar({ char, index, animationKey, skipEntrance, speed, playClick }: SplitFlapCharProps) {
  ...
  const bgColor = isSettled ? "hsl(0, 0%, 0%)" : "rgba(20, 184, 166, 0.1)"
  const textColor = isSettled ? "#ffffff" : "#14b8a6" // Changed from orange to teal
  ...
  if (isSpace) {
    return (
      <div
        style={{
          width: "0.3em",
          fontSize: "clamp(1.2rem, 7vw, 10rem)",
        }}
      />
    )
  }
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden flex items-center justify-center font-press-start will-change-transform"
      style={{
        fontSize: "clamp(1.2rem, 7vw, 10rem)",
        width: "0.65em",
        height: "1.05em",
    ...
    >
      ...
      <motion.div
        ...
        className="absolute inset-x-0 top-0 bottom-1/2 origin-bottom overflow-hidden will-change-transform"
        style={{
          backgroundColor: bgColor,
          transformStyle: "preserve-3d",
